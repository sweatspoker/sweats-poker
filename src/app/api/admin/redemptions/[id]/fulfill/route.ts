 ...
 *   Body: { admin_user_id, idempotency_key? }
 *   Calls public.redemptions_fulfill_request.
 */
export async function POST(
  ...
  const { admin_user_id, idempotency_key } = body as {
  ...
  const { data, error } = await admin.rpc("redemptions_fulfill_request", {
    p_request_id: id,
    p_admin_user_id: admin_user_id,
    p_idempotency_key: idempotency_key ?? `fulfill:${id}:${randomUUID()}`,
  });
  if (error) {
