...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await admin.rpc("sessions_no_show_cancel", {
    p_session_id: session_id, p_admin_user_id: admin_user_id, p_reason: body.reason ?? "player_no_show",
  });
  if (error) {
    const msg = error.message ?? "unknown";
    if (msg.includes("session_not_found")) return NextResponse.json({ error: "session_not_found" }, { status: 404 });
    if (msg.includes("session_terminal")) return NextResponse.json({ error: msg }, { status: 409 });
