-- 0079: bump get_my_recent_activity's p_limit cap from 200 to 1000 so
-- the wallet "Activities" tab can paginate past the original 25-row
-- view via a "Show more" button. Body is otherwise byte-identical to
-- migration 0066 - only the `least(p_limit, 200)` clamp changed.
